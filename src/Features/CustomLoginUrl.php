...
final class CustomLoginUrl extends AbstractFeature {
	...
	protected function registerHooks(): void {
		// Request interception (early hook).
	...
	public function interceptRequest(): void {
		...
		// Direct access to wp-login.php or wp-register.php.
		if ( strpos( $requestUri, 'wp-login.php' ) !== false || strpos( $requestUri, 'wp-register.php' ) !== false ) {
			...
			// phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited -- Intentionally overriding for routing.
			$pagenow = 'index.php';

			return;
		}

		// Custom login slug accessed.
		$customPath  = $this->userTrailingSlashit( '/' . $this->loginSlug );
		$requestPath = untrailingslashit( $path );

		if ( untrailingslashit( home_url( $this->loginSlug, 'relative' ) ) === $requestPath ) {
	 ...
	 * @return string
	 */
	private function userTrailingSlashit( string $path ): string {
